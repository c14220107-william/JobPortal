...
@section('content')
    <div class="max-w-3xl mx-auto p-6 bg-white shadow-lg rounded-lg mt-8">
        ...
        @if (Auth::check() && Auth::user()->role === 'user')
            <form action="{{ route('job_vacancies.apply', $jobVacancy->id) }}" method="POST" class="bg-gray-100 shadow-md rounded px-8 pt-6 pb-8 mb-6">
                @csrf
                <div class="mb-4">
        ...
        @else
            <p class="text-gray-700">Please <a href="{{ route('login') }}" class="text-green-600 hover:underline">login</a> to apply for this job.</p>
        @endif
    </div>
@endsection 
