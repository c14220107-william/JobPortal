...
<div class="p-6">
    <!-- Header Section -->
    <div class="mb-8">
        <h2 class="text-3xl font-bold text-gray-800 tracking-wide">Edit Lowongan</h2>
        <p class="text-gray-600 mt-2">Ubah informasi lowongan pekerjaan</p>
    </div>
                ...
                <div>
                    ...
                    <select name="id_position" id="id_position"
                        ...
                        @foreach($positions as $position)
                            <option value="{{ $position->id }}" {{ old('id_position', $jobVacancy->id_position) == $position->id ? 'selected' : '' }}>
                                {{ $position->title }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="id_location" id="id_location"
                        ...
                        @foreach($locations as $location)
                            <option value="{{ $location->id }}" {{ old('id_location', $jobVacancy->id_location) == $location->id ? 'selected' : '' }}>
                                {{ $location->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="id_department" id="id_department"
                        ...
                        @foreach($departments as $department)
                            <option value="{{ $department->id }}" {{ old('id_department', $jobVacancy->id_department) == $department->id ? 'selected' : '' }}>
                                {{ $department->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="available_from_date" id="available_from_date"
                        ...
                        value="{{ old('available_from_date', $jobVacancy->available_from_date) }}" required>
                ...
                <div>
                    ...
                    <input type="date" name="available_to_date" id="available_to_date"
                        ...
                        value="{{ old('available_to_date', $jobVacancy->available_to_date) }}" required>
                ...
                <div>
                    ...
                    <input type="number" name="kebutuhan" id="kebutuhan" min="1"
                        ...
                        value="{{ old('kebutuhan', $jobVacancy->kebutuhan) }}" required>
                    @error('kebutuhan')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                ...
                <div class="mt-1">
                    <textarea name="requirement" id="requirement" rows="4"
                        ...
                        required>{{ old('requirement', $jobVacancy->requirement) }}</textarea>
                </div>
                @error('requirement')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <div class="mt-1">
                    <textarea name="benefit" id="benefit" rows="4"
                        ...
                        required>{{ old('benefit', $jobVacancy->benefit) }}</textarea>
                </div>
                @error('benefit')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <div class="mt-1">
                    <textarea name="additional_info" id="additional_info" rows="3"
                        class="block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm @error('additional_info') border-red-300 text-red-900 @enderror">{{ old('additional_info', $jobVacancy->additional_info) }}</textarea>
                </div>
